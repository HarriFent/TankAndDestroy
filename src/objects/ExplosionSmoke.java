...
public class ExplosionSmoke extends GameObject {

    private double scaleCount = 0;

    public ExplosionSmoke(double x, double y, ID id, TankGame game) {
        super(x, y, id, game);
        stateTick = 255;
    }
    ...
    public void render(Graphics g, SpriteSheet ss) {
        ...
        if (stateTick > 235) {
            g2d.drawImage(ss.grabImage(SpriteList.smokeOrange3), tx, game);
        } else if (stateTick > 215) {
            g2d.drawImage(ss.grabImage(SpriteList.smokeOrange4), tx, game);
        } else {
            g2d.drawImage(ss.grabImage(SpriteList.smokeOrange5), tx, game);
        }
